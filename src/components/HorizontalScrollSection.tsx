...
const HorizontalScrollSection = () => {
  ...
  return (
    <section ref={sectionRef} className="relative h-screen overflow-hidden">
      <div ref={scrollRef} className="flex h-full items-center">
        {items.map((item, index) => (
          <div
            key={index}
            className="scroll-card min-w-full flex items-center justify-center relative"
            style={{ height: '60vh' }}
          >
            <div className={`absolute inset-0 ${item.color} opacity-10`} />
            <h2 className="text-[20vw] font-black tracking-tighter relative z-10 leading-none">
              {item.title}
            </h2>
          </div>
        ))}
